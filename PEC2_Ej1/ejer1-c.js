...
const callAsyncFindOune = async () => {
    ...
    try {
         ...
         */
        const result1 = await findOne(users, { key: 'name', value: 'Carlos' });
        onSuccess(result1); //En el cas que la promesa es resolgui correctament, es llançarà la funció onSuccess().
    } catch (err) {
        onError(err); //En el cas que la promesa es rebutgi, es capturarà l'error i es llançarà la funció onError().
    }
    ...
    try{
         ...
         */
        const result2 = await findOne(users, { key: 'name', value: 'Fermin' })
        onSuccess(result2);
    }catch (err){
        onError(err);
    }
